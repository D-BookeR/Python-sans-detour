...
def creer_csv(nom_classeur, liste_elts):
    feuille = pandas.DataFrame(liste_elts)
    feuille.to_csv(nom_classeur, index=False, header=False)
    feuille.to_clipboard()


if __name__ == '__main__':
    nom_dossier = '/tmp/opencv'
    l_fichiers, l_dossiers = explore_dossier(nom_dossier)
    creer_classeur("/tmp/mon_dossier.xlsx",
                   "opencv",
                   l_fichiers,
                   l_dossiers)
    creer_csv("/tmp/mes_fichiers.csv", l_fichiers)
    creer_csv("/tmp/mes_dossiers.csv", l_dossiers)
    selection = selection_fichier(l_fichiers, 2 ** 19)
    if len(selection) > 0:
